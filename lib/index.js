...
function concat(fetches) {
    ...
    return new Bimap(Product.create(fetches), function (a2) { return fetches.map(function () { return a2; }); }, function (ps) { return Object.assign.apply(null, [{}].concat(ps)); });
}
...
function sequence(fetches, as) {
    var itok = Object.keys(fetches);
    var observables = itok.map(function (k) { return observeAndRun(fetches[k], as[k]); });
    return Observable_1.Observable.combineLatest.apply(Observable_1.Observable, observables.concat([function () {
            var values = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                values[_i] = arguments[_i];
            }
            var out = {};
            itok.forEach(function (k, i) {
                out[k] = values[i];
            });
            return out;
        }]));
...
function sequenceSync(fetches, as) {
    var out = {};
    for (var k in fetches) {
...
function Query(options) {
    ...
    var leaf = Leaf.create(options.fetch, options.cacheStrategy, new ObservableCache({ atok: options.atok }));
    var createQuery = function () {
        if (keysLength === 0) {
            return leaf;
        }
        else {
            var fetches = keys.map(function (k) { return dependencies[k]; });
            var params_1 = options.params;
            var paramsLength_1 = Object.keys(params_1).length;
            if (paramsLength_1 > 0) {
                var paramsFetch = Leaf.create(function (a) { return Promise.resolve(a); }, exports.refetch, new ObservableCache());
                fetches.push(paramsFetch);
            }
            var product = Product.create(fetches);
            var composition = Composition.create(product, leaf)(function (p) {
                var a = {};
                keys.forEach(function (k, i) {
                    a[k] = p[i];
                });
                if (paramsLength_1 > 0) {
                    for (var k in params_1) {
                        a[k] = p[keysLength][k];
                    }
                }
                return a;
            });
            var a2toa1 = function (a2) {
                var a1 = keys.map(function (k) { return a2[k]; });
                if (paramsLength_1 > 0) {
                    a1.push(a2);
                }
                return a1;
            };
            return new Bimap(composition, a2toa1, function_1.identity);
        }
    };
    var out = createQuery();

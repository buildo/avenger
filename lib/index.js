...
var Command = (function () {
    ...
    Command.create = function (fetch, invalidates) {
        return new Command(fetch, invalidates);
    };
    Command.prototype.run = function (a) {
        var _this = this;
        return this.fetch(a).then(function () {
            _this.invalidates.forEach(function (f) { return f.run(a); });
        });
    };
    return Command;
}());
...
var Commands = (function () {
    ...
    Commands.prototype.run = function (a) {
        return Promise.all(this.commands.map(function (command) { return command.run(a); })).then(function () { return undefined; });
    };
    return Commands;
}());

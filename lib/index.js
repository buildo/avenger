...
function sequence(fetches, as) {
    ...
    var product = Product.create(itok.map(function (k) { return fetches[k]; }));
    var observable = observeAndRun(product, itok.map(function (k) { return as[k]; })).map(function (_a) {
        var loading = _a.loading, data = _a.data;
        return data.fold(function () { return LOADING; }, function (ps) {
            var outData = {};
            itok.forEach(function (k, i) { outData[k] = ps[i]; });
            return new CacheEvent(loading, Option_1.some(outData));
        });
    });
    return observable;
}
...
function sequenceSync(fetches, as) {
    ...
    var itok = Object.keys(fetches);
    var product = Product.create(itok.map(function (k) { return fetches[k]; }));
    var _a = product.getCacheEvent(itok.map(function (k) { return as[k]; })), loading = _a.loading, data = _a.data;
    return data.fold(function () { return LOADING; }, function (ps) {
        var outData = {};
        itok.forEach(function (k, i) { outData[k] = ps[i]; });
        return new CacheEvent(loading, Option_1.some(outData));

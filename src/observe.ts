...
import { setoidStrict, setoidShallow, setoidJSON, JSONT } from './Strategy';
...
export const sequenceRecordQueryResult = sequence(queryResult);
...
export function observeJSON<A, L extends JSONT, P extends JSONT>(
  query: ObservableQuery<A, L, P>,
  input: A
): Observable<QueryResult<L, P>> {
  return _observe(query, input).pipe(

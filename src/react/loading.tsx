...
export function loading<WP>(
  Component: React.ComponentClass<WP>,
  notReady: () => JSX.Element
): <OP>(f: (ownProps: OP) => Option<WP>) => React.ComponentClass<OP> {
  return function<OP>(f: (ownProps: OP) => Option<WP>) {
    return class LoadingWrapper extends React.Component<OP, void> {
      ...
      render() {
        return f(this.props).fold(notReady, props => <Component {...props as any} />)
      }
    }
  }
}

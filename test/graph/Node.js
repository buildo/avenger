// copy-pasted from LOL web-av branch

import t from 'tcomb';
import difference from 'lodash/difference';
import uniq from 'lodash/uniq';
import identity from 'lodash/identity';
import { refetch } from '../../src/cache/strategies';
import { compose, product } from '../../src/fetch/operators';
...
export default function Node({ fetch: _fetch, ...q }) {
  const compound = q.id;
  const strategy = q.strategy || refetch;
  const cache = new ObservableCache({ name: compound });
  const fetch = cacheFetch(_fetch, strategy, cache);

  if (t.Nil.is(q.dependencies)) {

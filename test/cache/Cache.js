  ...
  describe('Cache', () => {
    ...
    describe('getPromise', () => {
      ...
      it('should fetch when a cache miss occours', () => {
        const fetch = sinon.spy(a => Promise.resolve(2 * a))
        const cache = new Cache()
        return cache.getPromise(1, available, fetch).then(p => {
          ...
          assert.strictEqual(hit.blocked, undefined)
          assert.strictEqual(typeof hit.done, 'object')
          assert.strictEqual(hit.done.value, 2)
        })
      })
